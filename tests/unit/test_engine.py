...
class MockProvider(TextProvider):
    ...
    async def judge(self, content: str, criteria: list[Criterion]) -> JudgmentResult:
        ...
        return JudgmentResult(
            scores=[JudgmentScore(criterion=c.name, score=min(base, c.max_score), reasoning="ok") for c in criteria]
        )
...
class TestEngine:
    ...
    async def test_optimize_improves_score(self, eval_config: EvaluationConfig) -> None:
        ...
        result = await engine.optimize(content="You are a helpful assistant.", evaluation=eval_config, max_iterations=3)
    ...
    async def test_optimize_stops_at_max_iterations(self, eval_config: EvaluationConfig) -> None:
        provider = MockProvider()
        engine = Engine(provider=provider, judge_provider=provider)
        result = await engine.optimize(content="Short", evaluation=eval_config, max_iterations=2)
    ...
    async def test_optimize_with_target_score(self, eval_config: EvaluationConfig) -> None:
        provider = MockProvider()
        engine = Engine(provider=provider, judge_provider=provider)
        result = await engine.optimize(content="Short", evaluation=eval_config, max_iterations=10, target_score=3.0, convergence_strategy="target")
    ...
    async def test_optimize_callback(self, eval_config: EvaluationConfig) -> None:
        ...
        await engine.optimize(content="Test", evaluation=eval_config, max_iterations=2, on_iteration=on_iteration)
    ...
    async def test_optimize_with_stagnation(self, eval_config: EvaluationConfig) -> None:
        ...
        result = await engine.optimize(
            ...
            evaluation=eval_config, max_iterations=20, stagnation_limit=2,
        )
